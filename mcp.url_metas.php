...
class Url_metas_mcp {
	...
	public function __construct( $switch = TRUE )
	{
		...
		if ($switch){
			switch ($IN->GBL('P'))
			{
				case 'add':			$this->edit_metas();
				break;

				case 'edit':		$this->edit_metas();
				break;

				case 'view':		$this->view_metas();
				break;

				case 'create':		$this->update_metas("create");
				break;

				case 'update':		$this->update_metas();
				break;
	...
	function url_metas_header($page="",$msg="")
	{
		...
		$r = $this->url_metas_nav(
					array(
						...
						'edit_metas'    => '#',
						'default_metas'         => array('P' => 'defaults'),
					),
					$page
				);
...
EOT;
	...
	/**  View URL Metas
	/** ----------------------------------------*/
	function view_metas($msg = "")
	{
		global $DB, $IN, $DSP, $LANG;

		$this->url_metas_header("view_metas",$msg);
		$DSP->crumb .= $DSP->crumb_item($LANG->line('view_metas'));
	...
	/**  Add URL Metas
	/** ----------------------------------------*/
	function default_metas($msg="")
	{
		global $DB, $IN, $DSP, $LANG;


		$this->url_metas_header("default_metas",$msg);
		$DSP->crumb .= $DSP->crumb_item($LANG->line("default_metas"));

		$query          = $DB->query("SELECT * FROM exp_url_metas WHERE `def` = 'YES'");

	//                  $form_url       = 'C=modules'.AMP.'M=Freeform'.AMP.'P=update_metas';
	...
	$r              .= $DSP->qdiv('tableHeading', $LANG->line("default_metas"));
	...
	/**  Add URL Metas
	/** ----------------------------------------*/
	function edit_metas($msg="")
	{
		global $DB, $IN, $DSP, $LANG;

		$update_mode    = ( $IN->GBL('url_id') ) ? 'update': 'create';
		$_pagetitle = $update_mode == "update" ? "edit_metas" : "add_url_metas";
	...
	/**  Add URL Metas
	/** ----------------------------------------*/
	function update_metas($type="update")
	{
		global $DB, $IN, $DSP, $LANG;
